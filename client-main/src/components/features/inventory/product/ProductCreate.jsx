...
const ProductForm = () => {
  ...
  const validationPatterns = {
    ...
    price: /^\d+(\.\d{1,2})?$/,
    quantity: /^\d{1,10}$/,
    // MODIFIED: Reinforced description regex to ensure multiple spaces are allowed
    ...
    quantityAlert: /^\d{1,6}$/,
    // NEW: Added brand regex to allow spaces
    brand: /^[a-zA-Z0-9\s\-]{0,100}$/,
  };

  // MODIFIED: Enhanced sanitization to preserve spaces and added debug logging
  const sanitizeInput = (value, preserveSpaces = false) => {
    ...
    // NEW: Debug logging to track input sanitization
    console.log(`Sanitizing input: Original="${value}", Sanitized="${sanitized}", PreserveSpaces=${preserveSpaces}`);
    return sanitized;
  };
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      // NEW: Added brand validation
      case "brand":
        return validationPatterns.brand.test(value)
          ? ""
          : t("invalidBrandFormat");
      default:
        return "";
    }
  };

  // MODIFIED: Enhanced inputChange to preserve spaces for all variant inputs
  const inputChange = (key, value) => {
    // const preserveSpaces = key !== "Expiry"; // Preserve spaces for all variants except Expiry
    const sanitizedValue = sanitizeInput(value, key !== "Expiry");
    ...
    if (step === 3) {
      ...
      if (key === "Expiry") {
        ...
        // NEW: Preserve spaces in variant values
      ...
      // NEW: Debug logging for variant input
      // console.log(`Variant input: Key=${key}, Value="${sanitizedValue}", Parsed=${JSON.stringify(parsedValues)}`);
    } else {
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      // NEW: Added brand validation for Service item type
      // if (formData.itemType === "Service" && formData.brand) {
    ...
    // NEW: Debug logging for step validation
    // console.log(`Step ${step} validation: IsValid=${isValid}, Errors=`, errors);
    return isValid;
  };
  ...
  // MODIFIED: Preserve spaces and added debug logging
  const handleChange = (e) => {
    const { name, value } = e.target;
    // const preserveSpaces = ["productName", "description", "seoTitle", "seoDescription", "brand"].includes(name);
    const sanitizedValue = sanitizeInput(value, ["productName", "description", "seoTitle", "seoDescription", "brand"].includes(name));
    const error = validateField(name, sanitizedValue);
    setFormErrors((prev) => ({ ...prev, [name]: error }));
    setFormData((prev) => ({ ...prev, [name]: sanitizedValue }));
    // NEW: Debug logging for input changes
    console.log(`Input changed: Name=${name}, Value="${sanitizedValue}", Error="${error}"`);
  };
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(options);
      // NEW: Debug logging for fetched categories
  ...
  const fetchUnits = async () => {
    try {
      ...
      setUnitsOptions(options);
      // NEW: Debug logging for fetched units
      // console.log("Fetched units:", options);
    } catch (error) {
      console.error("Failed to fetch active units:", error);
  ...
  const fetchSubcategoriesByCategory = async (categoryId) => {
    try {
      ...
      setSubcategories(options);
      // NEW: Debug logging for fetched subcategories
  ...
  const fetchBrands = async () => {
    try {
      ...
      setBrandOptions(options);
      // NEW: Debug logging for fetched brands
      // console.log("Fetched brands:", options);
    } catch (error) {
      console.error("Failed to load active brands:", error);
  ...
  useEffect(() => {
    const fetchActiveSuppliers = async () => {
      try {
        ...
        setOptions(formattedOptions);
        // NEW: Debug logging for fetched suppliers
        // console.log("Fetched suppliers:", formattedOptions);
      } catch (err) {
        console.error("Error fetching active suppliers:", err);
    ...
    const fetchWarehouses = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setOptionsWare(formatted);
          // NEW: Debug logging for fetched warehouses
    ...
    const fetchHSN = async () => {
      try {
        ...
        if (res.data.success) {
          ...
          setOptionsHsn(formatted);
          // NEW: Debug logging for fetched HSN codes
          // console.log("Fetched HSN codes:", formatted);
        }
      } catch (err) {
        console.error("Error fetching HSN:", err);
  ...
  const generateSKU = () => {
    const category = formData.category || "GEN";
    // MODIFIED: Preserve spaces in productName for SKU generation
    // const name = formData.productName ? formData.productName.replace(/\s+/g, '-') : "PRD";
    const name = formData.productName || "PRD";
    const randomNum = Math.floor(Math.random() * 9000) + 1000;
    const sku = `${category.toUpperCase().slice(0, 3)}-${name.toUpperCase().slice(0, 3)}-${randomNum}`;
    setFormErrors((prev) => ({ ...prev, sku: "" }));
    setFormData((prev) => ({ ...prev, sku }));
    // NEW: Debug logging for SKU generation
    // console.log(`Generated SKU: ${sku}, from ProductName="${formData.productName}"`);
  };
  ...
  // MODIFIED: Enhanced form submission to preserve spaces and log payload
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validateStep()) {
    ...
    // MODIFIED: Preserve spaces for brand
    // formPayload.append("brand", selectedBrands?.value || sanitizeInput(formData.brand, true));
    formPayload.append("brand", selectedBrands?.value || "");
    formPayload.append("category", selectedCategory?.value || "");
    formPayload.append("subCategory", selectedsubCategory?.value || "");
    if (selectedSupplier?.value && typeof selectedSupplier.value === 'string' && selectedSupplier.value.trim() !== '') {
    ...
    // NEW: Debug logging for form payload
    // const payloadDebug = {};
    // for (let [key, value] of formPayload.entries()) {
    //   payloadDebug[key] = value;
    // }
    // console.log("Form submission payload:", payloadDebug);
    ...
    <div className="page-wrapper mt-4">
      <div className="content">
        <div className="page-header">
          ...
          <div className="page-btn mt-0">
            <div className="d-flex gap-2">
              <Link to="/product">
                <a className="btn btn-primary">{t("backToProduct")}</a>
              </Link>
            </div>
          </div>
        </div>
            ...
            {step === 0 && (
              <div className="accordion-body">
                ...
                {formData.itemType === "Good" ? (
                    ...
                    {showHSNModal && selectedHSN && (
                      <div className="modal fade show" style={{ display: 'block', background: 'rgba(0,0,0,0.3)' }} tabIndex="-1">
                        <div className="modal-dialog modal-dialog-centered">
                          <div className="modal-content" style={{ borderRadius: '12px', boxShadow: '0 4px 24px rgba(0,0,0,0.15)' }}>
                            <div className="modal-header bg-info text-white" style={{ borderTopLeftRadius: '12px', borderTopRightRadius: '12px' }}>
                              <h5 className="modal-title">{t("hsnDetails")}</h5>
                              <button type="button" className="btn-close" onClick={() => setShowHSNModal(false)}></button>
                            </div>
                            <div className="modal-body">
                              <div className="mb-3">
                                <label className="fw-bold">{t("hsnCode")}:</label>
                                <div className="fs-5 text-primary">{selectedHSN.label.split(' - ')[0]}</div>
                              </div>
                              <div className="mb-3">
                            ...
                            <div className="modal-footer">
                              <button type="button" className="btn btn-secondary" onClick={() => setShowHSNModal(false)}>{t("close")}</button>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <>
                        ...
                        <div className="col-sm-6 col-12 mb-3">
                          ...
                          <div className="form-check form-check-inline">
                            <input
                              ...
                              id="serial"
                            />
                            <label className="form-check-label" htmlFor="serial">{t("serialNo")}</label>
                          </div>
                          <div className="form-check form-check-inline">
                            <input
                              className="form-check-input"
                              ...
                              id="batch"
                            />
                            <label className="form-check-label" htmlFor="batch">{t("batchNo")}</label>
                          </div>
                          <div className="form-check form-check-inline">
                            <input
                              className="form-check-input"
                              ...
                              id="status"
                            />
                            <label className="form-check-label" htmlFor="status">{t("status")}</label>
                          </div>
                        </div>
                        ...
                        {formData.trackType === "status" && (
                          <div className="form-check mb-3">
                            ...
                            />
                            <label className="form-check-label" htmlFor="returnable">{t("returnable")}</label>
                          </div>
                        )}
                      </>
                    )}
                  ...
                  <>
                    <div>
                      <label className="block text-sm font-medium">{t("expireDate")}</label>
                      <input
                        type="date"
                        className="w-full p-2 border rounded"
                        name="expirationDate"
        ...
        <CategoryModal
          ...
          categoryName={categoryName}
          categorySlug={categorySlug}
          onCategoryChange={(e) => setCategoryName(sanitizeInput(e.target.value, true))}
