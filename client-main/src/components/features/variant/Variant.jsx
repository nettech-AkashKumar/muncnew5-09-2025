...
const Variant = ({ show, handleClose }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${selectedVariants.length} selected variants?`))
      return;
    ...
    try {
      ...
      setVariantData((prev) =>
        prev.filter((item) => !selectedVariants.includes(item.id))
      );
      setSelectedVariants([]);
    } catch (err) {
      console.error("Error bulk deleting:", err);
      setError("Failed to delete selected variants");
    }
  };

  useEffect(() => {
    setSelectedVariants((prev) =>
      prev.filter((id) => variantData.some((v) => v.id === id))
    );
  }, [variantData]);
          ...
          <ul className="table-top-head">
            <li>
              {selectedVariants.length > 0 && (
                ...
                >
                  Delete ({selectedVariants.length}) Selected
                </button>
              )}
            </li>
            <li>
            ...
            <div className="table-dropdown my-xl-auto right-content">
              <div className="dropdown">
                ...
                <ul className="dropdown-menu  dropdown-menu-end p-3">
                  <li>
                    <a value="all" className="dropdown-item rounded-1">
                      All
                    </a>
                  </li>
                  <li>
                    <a value="active" className="dropdown-item rounded-1">
                      Active
                    </a>
                  </li>
                  <li>
                    <a value="inactive" className="dropdown-item rounded-1">
                      Inactive
                    </a>
                  </li>
                </ul>
